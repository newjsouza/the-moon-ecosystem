...
import { Network, BrainCircuit, TerminalSquare } from 'lucide-react';
...
export function TopNav({ activeTab, setActiveTab }: TopNavProps) {
  const tabs = [
    ...
    { id: 'terminal', label: 'UPLINK & LOGS', icon: TerminalSquare, color: 'text-cyber-purple', bgColor: 'bg-cyber-purple' }
  ];

  return (
    <div className="w-full h-16 glass-panel border-b border-white/5 mb-6 px-6 flex items-center justify-between">
      <div className="flex items-center gap-3">
        <div className="h-8 w-8 rounded bg-cyber-accent/20 border border-cyber-accent flex items-center justify-center animate-pulse-slow">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-cyber-accent font-mono uppercase tracking-[0.2em]">Cyber-Agentic OS</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-mono text-cyber-success">SYS.ONLINE</span>
      </div>
    </div>
  );
}
