 ...
 *
 * State:
 * State file: /.gstack/browse.json (set via BROWSE_STATE_FILE env)
 * Log files: /.gstack/browse-{console,network,dialog}.log
...
async function flushBuffers() {
  ...
  try {
    ...
    if (newConsoleCount > 0) {
      ...
      await Bun.write(
        CONSOLE_LOG_PATH,
        (await Bun.file(CONSOLE_LOG_PATH).text().catch(() => '')) + lines
      );
    ...
    if (newNetworkCount > 0) {
      ...
      await Bun.write(
        NETWORK_LOG_PATH,
        (await Bun.file(NETWORK_LOG_PATH).text().catch(() => '')) + lines
      );
    ...
    if (newDialogCount > 0) {
      ...
      await Bun.write(
        DIALOG_LOG_PATH,
        (await Bun.file(DIALOG_LOG_PATH).text().catch(() => '')) + lines
      );
